...
void pendulum_loop(pendulum_ctx_t *ctx, float total_angle, float pendulum_angle)
{
    ...
    if (ctx->limit_tripped && ctx->state != STATE_IDLE && ctx->state != STATE_CALIB_DONE && ctx->state != STATE_CALIBRATE) {
...
static void do_calibration(pendulum_ctx_t *ctx)
{
    ...
    case 0: /* 正转中 (POSITIVE_DIR，平台右移)，检测右限位 (PA11) */
        if (LIMIT_RIGHT_IS_HIT()) {
            ...
            printf("CALIB: right = %.2f, seeking left...\r\n", (double)total_angle);
        }
        break;
...
static void do_move_to_mid(pendulum_ctx_t *ctx)
{
    ...
    /* 到达判定 */
    if (fabsf(error) < MOVE_ARRIVE_THRESH) {
        step_motor_stop(&motor);
        ...
        printf("MID: arrived at %.2f, starting disturb...\r\n", (double)total_angle);
        return;
    }
...
static void do_swing(pendulum_ctx_t *ctx)
{
    ...
    if (ANGLE_IS_UPSIDE(angle)) {
        step_motor_stop(&motor);
        ctx->state = STATE_IDLE;
        printf("SWING: upright! angle=%.1f, done\r\n", (double)angle);
        return;
    }
    ...
    /*----- 第五步：决定推力方向 -----*/
    ctx->swing_count++;
    printf("SWING: peak #%d at %.1f deg\r\n", ctx->swing_count, (double)a1);
